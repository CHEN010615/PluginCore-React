...
const appendHistory = (values: number[], nextValue: number) => [...values.slice(-7), clampPercent(nextValue)]
  ...
  series: [
    {
      ...
      showSymbol: false,
      smooth: false,
      lineStyle: { width: 4, color, cap: 'round', join: 'round' },
  ...
  <Box sx={{ width: 'var(--donut-size)', height: 'var(--donut-size)', mx: 'auto', mt: 'calc(var(--dashboard-gap) * 0.75)', position: 'relative' }}>
    ...
    <Box sx={{ position: 'absolute', inset: 'var(--donut-inner-inset)', borderRadius: '50%', bgcolor: '#F9FAFB', display: 'grid', placeItems: 'center' }}>
      <Box sx={{ textAlign: 'center' }}>
        <Typography sx={{ color, fontSize: 'var(--donut-value-font)', fontWeight: 800, lineHeight: 1 }}>
          {Math.round(value)}%
        </Typography>
        <Typography sx={{ mt: 'calc(var(--dashboard-gap) * 0.25)', color: '#6B7280', fontSize: 'var(--donut-label-font)', fontWeight: 800, letterSpacing: '0.04em' }}>
          {label}
        </Typography>
      </Box>
    </Box>
...
const StorageDeviceRow = ({
  ...
  <Box
    sx={{
      p: 'calc(var(--card-padding) * 0.78)',
      mb: isLast ? 0 : 'calc(var(--dashboard-gap) * 0.55)',
  ...
  >
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'var(--summary-icon-size) 1fr', sm: 'var(--summary-icon-size) 1fr auto' }, alignItems: 'center', gap: 'var(--dashboard-gap)' }}>
      ...
      <Typography sx={{ color: '#4B5563', fontSize: 'var(--body-font)', fontWeight: 700, gridColumn: { xs: '2', sm: 'auto' } }}>
    ...
    <Box sx={{ mt: 'calc(var(--dashboard-gap) * 0.45)', ml: { xs: 0, sm: 'calc(var(--summary-icon-size) + var(--dashboard-gap))' } }}>
      <EChart option={progressOption(device.usagePercent)} height="var(--progress-height)" />
    </Box>
  </Box>
)

const StorageDeviceTile = ({
  device,
  index,
  name
}: {
  ...
  >
    <Box sx={{ display: 'grid', gridTemplateColumns: 'var(--summary-icon-size) 1fr', alignItems: 'center', gap: 'calc(var(--dashboard-gap) * 0.7)', minWidth: 0 }}>
      ...
      <Box sx={{ minWidth: 0 }}>
        ...
        <Typography sx={{ color: '#4B5563', fontSize: 'var(--interface-font)', fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {formatBytes(device.used)} / {formatBytes(device.total)}
        </Typography>
      </Box>
    </Box>
...
const StorageDeviceRing = ({
  ...
  <Box
    sx={{
      minHeight: 0,
      height: '100%',
      minWidth: 'var(--storage-scroll-item-width)',
      px: 'calc(var(--card-padding) * 0.72)',
      py: 'var(--storage-ring-padding-y)',
      borderRadius: 'var(--small-radius)',
      border: '1px solid #E5E7EB',
      bgcolor: 'rgba(255, 255, 255, 0.7)',
      display: 'flex',
      ...
      gap: 'calc(var(--dashboard-gap) * 0.5)'
    }}
  >
    <Box sx={{ width: 'var(--storage-ring-size)', height: 'var(--storage-ring-size)', position: 'relative' }}>
      <EChart option={donutOption(device.usagePercent)} height="var(--storage-ring-size)" />
      <Box
        sx={{
          position: 'absolute',
          top: 'calc(var(--donut-inner-inset) * 0.7)',
          right: 'calc(var(--donut-inner-inset) * 0.7)',
          bottom: 'calc(var(--donut-inner-inset) * 0.7)',
          left: 'calc(var(--donut-inner-inset) * 0.7)',
          borderRadius: '50%',
          bgcolor: '#F9FAFB',
          display: 'flex',
          alignItems: 'center',
      ...
      >
        <Typography sx={{ color: '#0065CC', fontSize: 'var(--body-font)', fontWeight: 900, lineHeight: 1 }}>
          {Math.round(device.usagePercent)}%
        </Typography>
      </Box>
    </Box>
    <Box sx={{ width: '100%', minWidth: 0, textAlign: 'center' }}>
      <Typography sx={{ fontSize: 'var(--body-font)', fontWeight: 900, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</Typography>
      <Typography sx={{ mt: 'calc(var(--dashboard-gap) * 0.2)', color: '#4B5563', fontSize: 'var(--interface-font)', fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {formatBytes(device.used)} / {formatBytes(device.total)}
      </Typography>
    </Box>
  </Box>
...
function App() {
  ...
  const [history, setHistory] = useState<MetricHistory>({
    cpu: emptyHistory,
    memory: emptyHistory,
    download: emptyHistory,
    upload: emptyHistory
  })
  const [loading, setLoading] = useState({ os: false, node: false, performance: false })
  const [error, setError] = useState<{ os: string | null; node: string | null; performance: string | null }>({
    os: null,
    node: null,
    performance: null
  })
  ...
  const handleGetPerformanceInfo = async (silent = false) => {
    ...
    try {
      const info = await window.electronAPI.invoke('system:performance') as PerformanceInfo
      ...
      const memoryUsage = parsePercent(info.memory.usagePercent)
      const rxRate = info.network.rxBytesPerSec ?? 0
      const txRate = info.network.txBytesPerSec ?? 0

      setPerformanceInfo(info)
      setHistory(prev => ({
        cpu: appendHistory(prev.cpu, cpuUsage),
        memory: appendHistory(prev.memory, memoryUsage),
        download: [...prev.download.slice(-7), rxRate],
        upload: [...prev.upload.slice(-7), txRate]
      }))
    } catch {
  ...
  const handleGetNodeInfo = async () => {
    ...
    try {
      ...
      setError(prev => ({ ...prev, node: t('errors.node') }))
    } finally {
  ...
  const memoryUsage = parsePercent(performanceInfo?.memory.usagePercent)
  ...
  const downloadRate = formatRate(performanceInfo?.network.rxBytesPerSec)
  const uploadRate = formatRate(performanceInfo?.network.txBytesPerSec)
  const networkInterfaces = performanceInfo?.network.interfaces ?? []
  const isEnglish = i18n.language === 'en'
  const changeLanguage = (checked: boolean) => {
    void i18n.changeLanguage(checked ? 'en' : 'zh')
  }
  const storageDevices = performanceInfo?.storage?.length
    ? performanceInfo.storage
    : [
        { name: t('hardware.driveC'), total: 1024 ** 4, used: 450 * 1024 ** 3, usagePercent: 45 },
        { name: t('hardware.driveD'), total: 4 * 1024 ** 4, used: 2 * 1024 ** 4, usagePercent: 50 }
      ]
  ...
  return (
    <Box
      ...
      sx={{
        ...
        color: '#111827',
        background:
          'radial-gradient(circle at 15% 0%, rgba(59, 130, 246, 0.08), transparent 28%), linear-gradient(135deg, #FFFFFF 0%, #F8FAFC 48%, #F4F7FF 100%)'
      }}
    >
      <TitleBar />
      <Box
        ...
        <Box className={styles.contentFrame} sx={{ mx: 'auto', display: 'flex', flexDirection: 'column' }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 'var(--dashboard-gap)', flexWrap: 'wrap', mb: 'var(--dashboard-gap)' }}>
            <Box>
              ...
              <Typography sx={{ mt: 'calc(var(--dashboard-gap) * 0.2)', fontSize: 'var(--page-title-font)', fontWeight: 800, lineHeight: 1.08 }}>
                {t('app.title')}
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', gap: 'var(--control-gap)', flexWrap: 'wrap', alignItems: 'center' }}>
              <Box
                sx={{
                  ...
                  py: 'var(--control-pad-y)',
                  borderRadius: 'var(--small-radius)',
                  bgcolor: 'rgba(255, 255, 255, 0.72)',
                  border: '1px solid #E5E7EB'
                }}
              >
                <Typography sx={{ color: isEnglish ? '#6B7280' : '#1D4ED8', fontSize: 'var(--body-font)', fontWeight: 800 }}>
                  {t('language.zh')}
                </Typography>
                <Switch
                  size="small"
                  checked={isEnglish}
                  onChange={event => changeLanguage(event.target.checked)}
                  inputProps={{ 'aria-label': t('language.switchLabel') }}
                />
                <Typography sx={{ color: isEnglish ? '#1D4ED8' : '#6B7280', fontSize: 'var(--body-font)', fontWeight: 800 }}>
                  {t('language.en')}
                </Typography>
              </Box>
              <Chip
                label={loading.os || loading.performance || loading.node ? t('status.syncing') : t('status.live')}
                sx={{ bgcolor: '#E8F7EF', color: '#047857', fontWeight: 800, borderRadius: 2 }}
              />
              <Chip
                label={performanceInfo?.hostname ?? t('status.localhost')}
                sx={{ bgcolor: '#EEF2FF', color: '#1D4ED8', fontWeight: 800, borderRadius: 2 }}
              />
            </Box>
          </Box>

          {(error.os || error.performance || error.node) && (
            <Alert severity="error" sx={{ mb: 'var(--dashboard-gap)', borderRadius: 'var(--small-radius)' }}>
              {error.os || error.performance || error.node}
            </Alert>
          )}
          ...
          <Box
            ...
            sx={{
              ...
              gridTemplateRows: { xs: 'none', md: 'minmax(0, 1.55fr) minmax(0, 1.05fr) minmax(0, 0.48fr) minmax(0, 0.36fr)' },
          ...
          >
            <Paper elevation={0} sx={{ ...cardSx, gridColumn: { xs: '1', md: 'span 4' } }}>
              ...
              <Box sx={{ mt: 'calc(var(--dashboard-gap) * 0.75)', color: '#4B5563', fontSize: 'var(--body-font)', fontWeight: 600 }}>
                <Typography sx={{ fontWeight: 800, color: '#111827' }}>
                  {performanceInfo?.cpu.model ?? t('hardware.processorLoading')}
                </Typography>
                <Typography sx={{ mt: 'calc(var(--dashboard-gap) * 0.3)' }}>
                  {performanceInfo ? `${performanceInfo.cpu.speed} MHz · ${t('metrics.uptime', { hours: uptimeHours })}` : t('status.waitingTelemetry')}
                </Typography>
              </Box>
            </Paper>
            ...
            <Paper elevation={0} sx={{ ...cardSx, gridColumn: { xs: '1', md: 'span 4' } }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 'calc(var(--dashboard-gap) * 0.65)', mt: 'calc(var(--dashboard-gap) * 0.9)', flexWrap: 'wrap' }}>
                <Typography sx={{ color: memoryColor, fontSize: 'var(--metric-font)', fontWeight: 900, lineHeight: 1 }}>
                  {formatBytes(performanceInfo?.memory.used).replace(/ (GB|TB)$/, '')}
                </Typography>
                <Typography sx={{ color: '#6B7280', fontSize: 'var(--metric-sub-font)', fontWeight: 800 }}>
                  / {formatBytes(performanceInfo?.memory.total)}
                </Typography>
              </Box>
              <Chip
                label={`${Math.round(memoryUsage)}% - ${memoryStatus}`}
                sx={{ mt: 'calc(var(--dashboard-gap) * 0.6)', bgcolor: memoryUsage >= 80 ? '#C45A00' : '#D1FAE5', color: memoryUsage >= 80 ? '#FFFFFF' : '#065F46', fontWeight: 900, borderRadius: 'var(--small-radius)' }}
              />
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 'calc(var(--dashboard-gap) * 0.6)' }}>
                <Typography sx={{ fontWeight: 800 }}>
                  <Box component="span" sx={{ display: 'inline-block', width: 'var(--legend-dot-size)', height: 'var(--legend-dot-size)', mr: 'calc(var(--control-gap) * 0.75)', borderRadius: '50%', bgcolor: memoryColor }} />
                  {t('metrics.active')}
                </Typography>
                <Typography sx={{ color: '#374151', fontWeight: 800 }}>
                  <Box component="span" sx={{ display: 'inline-block', width: 'var(--legend-dot-size)', height: 'var(--legend-dot-size)', mr: 'calc(var(--control-gap) * 0.75)', borderRadius: '50%', bgcolor: '#D1D5DB' }} />
                  {t('metrics.standby')}
                </Typography>
              </Box>
            </Paper>
            ...
            <Paper elevation={0} sx={{ ...cardSx, gridColumn: { xs: '1', md: 'span 6' }, display: 'flex', flexDirection: 'column' }}>
              ...
              <Box
                sx={{
                  mt: 'calc(var(--dashboard-gap) * 0.6)',
                  display: 'flex',
                  alignItems: 'flex-start',
                  gap: 'calc(var(--dashboard-gap) * 0.5)',
                  minHeight: 'var(--network-interface-height)',
                  pb: 'calc(var(--card-padding) * 0.35)'
                }}
              >
                <Typography sx={{ color: '#6B7280', fontSize: 'var(--interface-font)', fontWeight: 800, lineHeight: 1.5, flexShrink: 0 }}>
                  {t('metrics.interfaces')}:
                </Typography>
                  ...
                  {networkInterfaces.length ? (
                    networkInterfaces.map(networkInterface => (
                      <Box
                        ...
                        component="span"
                        sx={{
                          px: 'calc(var(--control-pad-x) * 0.55)',
                          py: '1px',
                          borderRadius: 'var(--small-radius)',
                          bgcolor: 'rgba(219, 234, 254, 0.8)',
                          color: '#1D4ED8',
                          fontSize: 'var(--interface-font)',
                          fontWeight: 800,
                          lineHeight: 1.45
                        }}
                      >
                    ...
                    <Typography sx={{ color: '#6B7280', fontSize: 'var(--interface-font)', fontWeight: 600, lineHeight: 1.5 }}>
                      {t('metrics.detectingAdapters')}
                    </Typography>
                  )}
                </Box>
              </Box>
                ...
                <Box sx={{ minHeight: 0 }}>
                  {storageDevices.map((device, index) => (
                    <StorageDeviceRow
                      ...
                      device={device}
                      index={index}
                      name={device.name === 'System Disk' ? t('hardware.systemDisk') : device.name}
                      isLast={index === storageDevices.length - 1}
                    />
                  ))}
                </Box>
              )}
              {storageDevices.length > 2 && storageDevices.length <= 4 && (
                <Box
                  sx={{
                    ...
                    gap: 'calc(var(--dashboard-gap) * 0.65)'
                  }}
                >
                  {storageDevices.map((device, index) => (
                    <StorageDeviceTile
              ...
              {storageDevices.length > 4 && (
                <Box
                  sx={{
                    ...
                    overflowX: 'auto',
                    overflowY: 'hidden',
                    pb: 'calc(var(--card-padding) * 0.55)',
                    scrollSnapType: 'x proximity',
                    '&::-webkit-scrollbar': {
                      height: 8
                    },
                    '&::-webkit-scrollbar-track': {
                      bgcolor: 'rgba(229, 231, 235, 0.75)',
                      borderRadius: 999
                    },
                    '&::-webkit-scrollbar-thumb': {
                      bgcolor: 'rgba(0, 101, 204, 0.45)',
                      borderRadius: 999
                    }
                  }}
                >
                  {storageDevices.map((device, index) => (
                    <Box key={`${device.name}-${index}`} sx={{ height: '100%', scrollSnapAlign: 'start' }}>
                      <StorageDeviceRing
                        device={device}
                        name={device.name === 'System Disk' ? t('hardware.systemDisk') : device.name}
                      />
            ...
            {[
              { icon: <Speed />, label: t('metrics.cpu'), value: performanceInfo?.cpu.model ?? 'Intel Core i9-13900K' },
              { icon: <Memory />, label: t('metrics.gpu'), value: t('hardware.gpuModel') },
              { icon: <DeveloperBoard />, label: t('metrics.motherboard'), value: osInfo ? `${osInfo.type} ${osInfo.arch}` : t('hardware.motherboardModel') }
            ].map(item => (
              <Paper key={item.label} elevation={0} sx={{ ...cardSx, gridColumn: { xs: '1', md: 'span 4' }, display: 'flex', alignItems: 'center', gap: 'var(--dashboard-gap)' }}>
                <Box sx={{ width: 'var(--summary-icon-size)', height: 'var(--summary-icon-size)', borderRadius: 'var(--small-radius)', bgcolor: '#E8F1FF', color: '#0065CC', display: 'grid', placeItems: 'center', flexShrink: 0, '& svg': { fontSize: 'var(--summary-icon-font)' } }}>
                  {item.icon}
                </Box>
                <Box sx={{ minWidth: 0 }}>
            ...
            <Paper elevation={0} sx={{ ...cardSx, py: 'calc(var(--card-padding) * 0.7)', gridColumn: '1 / -1', display: 'flex', justifyContent: 'space-between', gap: 'var(--dashboard-gap)', flexWrap: 'wrap' }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 'var(--dashboard-gap)' }}>
                ...
                <Typography sx={{ color: '#4B5563', fontSize: 'var(--body-font)', fontWeight: 700 }}>
                  Node {nodeInfo?.version || t('status.notAvailable')} · Electron {nodeInfo?.electron || t('status.notAvailable')}
                </Typography>
              </Box>
              <Typography sx={{ color: '#4B5563', fontSize: 'var(--body-font)', fontWeight: 700 }}>
                {t('metrics.loadAvg')}: {loadAverage} · {t('metrics.release')}: {osInfo?.release || '--'}
              </Typography>
            </Paper>
          </Box>
        </Box>
